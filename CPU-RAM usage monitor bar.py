...
class Application(tk.Tk, Configure_widgets):
    ...
    def make_bar_cpu_usage(self):
        ttk.Label(
            self.bar,
            text=
            f'physical cores: {self.cpu.cpu_count}, logical cores: {self.cpu.cpu_count_logical}',
    ...
    def make_minimal_win(self):
        ...
        ttk.Button(self, text='full', command=self.make_full_win,
                   width=5).pack(side=tk.RIGHT)

        ttk.Button(self, text='move', command=self.configure_win,
                   width=5).pack(side=tk.RIGHT)
